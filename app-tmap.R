...
tmap_options(basemaps = c(Map="Esri.NatGeoWorldMap", Imagery="Esri.WorldImagery"))
...
server = function(input, output) {
    ...
    output$map2 <- renderTmap({
        m <- tm_shape(bnd()) + tm_borders(col='black', lwd=2, group="FDA") 
        m
        #if (input$goButton) {
